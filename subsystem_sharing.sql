...
--
-- Host: 127.0.0.1
-- Generation Time: 2014-06-10 06:50:09
-- 服务器版本： 5.6.17-log
-- PHP Version: 5.5.12
...
INSERT INTO `crssResource` (`reso_id`, `reso_famile`, `reso_name`, `reso_update_date`, `reso_level`) VALUES
(1, 'book', 'cs_textbook', '2014-06-10', 1);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_countresource` AS select `a`.`cuz_id` AS `cuz_id`,`a`.`cuz_name` AS `cuz_name`,count(0) AS `count` from (`imscourse` `a` join `re_crssresource_imscourse` `b`) where (`a`.`cuz_id` = `b`.`cuz_id`) group by `b`.`cuz_id`;
